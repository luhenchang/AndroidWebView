    ...
    window.frame = {
        ...
        jumpSlide: function (i, time) {
            ...
            nextTick: function(callback) {
                var cb = function() {
                    TweenLite.ticker.removeEventListener("tick", cb)
                    callback && callback()
                }
                setTimeout(function() {
                    //requestAnimationFrame(cb);
                    TweenLite.ticker.addEventListener("tick", cb);
                })
            },
            restore: function(s){
                ...
                $(document.body).on("playTimeline.travel." + i, function(event, j){
                    ...
                    $(document.body).one("timelineStart.travel." + j, function(event, j){
                        if (slide.index == j) {
                            slide.goto(progress);
                        }
                    })
...
window.wxConfig.shareObj = {
    ...
    link: shareLink,
    imgUrl: doc_info.cover && /ppj\.io/.test(doc_info.cover) ? (doc_info.cover + "!wxshare") : doc_info.cover,
